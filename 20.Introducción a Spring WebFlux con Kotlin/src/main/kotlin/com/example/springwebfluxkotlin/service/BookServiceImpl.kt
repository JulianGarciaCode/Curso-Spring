package com.example.springwebfluxkotlin.service

import com.example.springwebfluxkotlin.domain.Book
import com.example.springwebfluxkotlin.repository.BookRepository
...
class BookServiceImpl(@Autowired val bookRepository: BookRepository) : BookService {
    ...
    override fun update(book: Book): Mono<Book> {
        return bookRepository.existById(book.id!!)
            .flatMap { exist -> if (exist) return@flatMap bookRepository.save(book)
